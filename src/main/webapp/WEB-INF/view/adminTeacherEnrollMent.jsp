...
<body>
	...
	<div class="main-content">
		...
		<div class="table-container">
			<h3>Teachers Assigned to Courses</h3>
			<div class="export-buttons">
    <form action="/LearingSYSLMS/Admin/ExportTeacherEnrollment" method="post">
        <button type="submit" class="btn btn-primary">Export PDF</button>
			...
			<table class="table">
			        ...
			        <tbody>
			            <c:forEach var="TeachersEntrollList" items="${TeachersEntrollList}" varStatus="status">
			                <tr>
			                    <td>${status.index + 1}</td>
			                    <td>${TeachersEntrollList.teacher.name}</td>
			                    <td>${TeachersEntrollList.course.title}</td>
			                    <td>${TeachersEntrollList.createAdminName}</td>
			                    <td>${TeachersEntrollList.status}</td>
			                </tr>
			            </c:forEach>
			        </tbody>
			    </table>
